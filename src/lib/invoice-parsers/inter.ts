...
export const interParser: BankParser = {
  ...
  detectBank: (text: string): boolean => {
    return /banco\s*inter|inter\s*mastercard/i.test(text);
  },
  ...
  parseTransactions: (text: string): ParsedTransaction[] => {
    ...
    // Inter format: DD/MM - Description R$ Amount
    // Example: 05/04 - UBER *TRIP R$ 15,90
    const transactionRegex = /^(\d{2}\/\d{2})\s+-\s+(.+?)\s+R\$\s*(-?[\d.,]+)$/;
