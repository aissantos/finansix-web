...
// Usa o tipo gerado automaticamente pelo Supabase
...
export function useCreateSubscription() {
  ...
  return useMutation({
    mutationFn: async (subscription: Omit<TablesInsert<'subscriptions'>, 'id' | 'household_id' | 'created_at' | 'updated_at'>) => {
...
export function useSubscriptionTotal() {
  ...
  const total = subscriptions?.reduce((sum, s) => {
    // Handle null/undefined amount values
    const amount = s.amount ?? 0;
    return sum + (s.is_active ? amount : 0);
  }, 0) ?? 0;
