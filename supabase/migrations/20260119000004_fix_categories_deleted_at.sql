-- Fix: Remove deleted_at references from categories table
-- Migration: 20260119000004_fix_categories_deleted_at.sql

-- Drop and recreate get_category_distribution without deleted_at on categories
...
COMMENT ON FUNCTION get_category_distribution IS 'Returns transaction distribution by category (no categories.deleted_at)';
