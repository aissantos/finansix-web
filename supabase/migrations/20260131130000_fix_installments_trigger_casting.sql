-- Migration to fix regression in explode_installments (missing enum cast)
-- This restores the fix from 20260114000005 that was lost in 20260130000001
-- And maintains the logic to handle single installments.
...
BEGIN
  ...
  FOR i IN 1..total_inst LOOP
    ...
    INSERT INTO installments (
      ...
      billing_date,
      due_date,
      installment_status::installment_status, -- FIX: Explicit cast here
      NOW(),
      NOW()
    );
  END LOOP;
...
-- Trigger definition remains valid (no change needed), but recreating ensures it uses the new function
